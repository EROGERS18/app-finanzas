...
export const AuthView: React.FC = () => {
  const { login, register, registeredUsers, loginWithGoogle } = useAuth();
  ...
  const handleGoogleLogin = async () => {
    ...
    if (!res.success) {
      if (mode === 'login') setLoginError(res.error || 'Error al conectar con Google.');
      else setRegError(res.error || 'Error al conectar con Google.');
    }
  };
    ...
    <div className="min-h-screen w-full flex items-center justify-center p-4 sm:p-6 bg-slate-50 dark:bg-[#070b14] text-slate-900 dark:text-white relative overflow-hidden transition-colors">
      ...
      <div className="w-full max-w-md z-10 space-y-6">
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 sm:p-8 shadow-xl shadow-slate-200/60 dark:shadow-none space-y-5">
          ...
          {/* Separador y Botón de Inicio de Sesión con Google */}
          <div className="pt-2 space-y-3">
            <div className="relative">
              ...
              <div className="relative flex justify-center text-xs">
                <span className="bg-white dark:bg-slate-900 px-3 text-slate-400 font-semibold uppercase text-[10px] tracking-wider">
                  o continuar con
                </span>
              </div>
            </div>
            ...
            <button
              ...
              className="w-full py-2.5 px-4 rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-750 text-slate-700 dark:text-slate-200 font-bold text-xs shadow-sm transition-all flex items-center justify-center gap-2.5 active:scale-98"
            >
